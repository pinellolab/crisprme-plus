...
**Tuned genome-wide (measured, all 3 registry sections).** A registry has 3 block-partitioned sections (record + group + string); the group blob is 75-80%, so the *total* block count is larger than the record count alone: **chr22 = 1,906 total blocks (~122 MB decompressed), chr1 (largest) = 10,373 (~660 MB).** The chr22 A/B, though, hit 3.66× at cache=512 (only 27% of chr22's total) — so the **working set a search actually re-touches is a fraction of the registry**, and 4096 comfortably covers even chr1's working set at a ~256 MB/reader ceiling (within the memory-capped pool's budget; the LRU only fills to touched blocks). **The mega sites-only index is `codec=RAW` (uncompressed) → no decompression, unaffected.**
