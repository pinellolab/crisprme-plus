...
**Measured (byte-identical throughout, same md5, 2085 rows).** Cache-only (with the *old* list-LRU): **326 s → 89 s (3.66×)**. Adding the O(1) LRU (below) roughly halved it again → **43 s**, so the combined 2.5.3 registry fix is **326 s → 43 s ≈ 7.6×** on the dense chr22 SNP post-analysis.

**Plateau sweep (clean, O(1) LRU).** Sweeping the cache on the dense chr22 search: cache=8 → 287 s, **cache=128 → 43 s (plateau reached)**, 512 → 43 s, 1906 → 42 s. So the **hot working set a search re-touches is tiny** (~128 of chr22's 1,906 total blocks). chr1's is ~5× (~640) → **4096 has large headroom**; and since the LRU only fills to *touched* blocks, actual memory is ~8-40 MB for typical searches (256 MB is just the pathological ceiling). (Total blocks, all 3 sections: chr22 = 1,906, chr1 = 10,373 — but only the working set is cached, so 4096 is ample.) **The mega sites-only index is `codec=RAW` → no decompression, unaffected.**
