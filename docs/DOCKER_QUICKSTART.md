...
> have — run `docker pull pinellolab/crisprme:v2.2.0-alpha.20` again to update. Skipping this makes an
...
```bash
docker run --rm -v "${PWD}:/DATA" -w /DATA pinellolab/crisprme:v2.2.0-alpha.20 \
...
apptainer pull crisprme.sif docker://pinellolab/crisprme:v2.2.0-alpha.20
...
```bash
docker run --rm -v "${PWD}:/DATA" -w /DATA pinellolab/crisprme:v2.2.0-alpha.20 crisprme.py download --what index --index-name NNN_3_hg38+hg38_1000G_HGDP --path /DATA
```
