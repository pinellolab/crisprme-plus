...
- **Fast mode is the default; `--full` for exact per-sample enumeration** *(new in 2.5.3)* — the SNP off-target analysis reports a small set of **worst-possible** representatives per variant window (reference + minimum-edit + maximum-CFD) instead of the 2^k IUPAC / observed-haplotype lattice, so a search stays tractable on any panel out of the box (full enumeration was measured **49 h+ without completing** on a 4×-density panel). **CFD is the exact worst case; CRISTA is best-effort.** The trade-off: **per-sample carriers, CONFIRMED cis phasing and exact joint AF are not computed in fast mode** — add **`--full`** (CLI) or pick **Full** in the web form for that per-sample resolution (recommended for genotyped panels / clinical validation). The shareable report states which mode was used. Fast is validated **lossless** for locus detection against the full path. ([methods](METHODS.md#5-search-space-control-for-high-variant-density-regions))
...
# ^ runs in FAST mode by default (worst-possible representatives; tractable on any panel).
