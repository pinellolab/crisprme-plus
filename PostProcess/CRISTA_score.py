...
def predict_crista_score(features_lst):
    ...
    # NOTE: the RF predict is NOT further parallelized here. The pickled predictors already
    # carry n_jobs=-1, so each RandomForest.predict already fans the tree traversal across all
    # cores -- adding a thread/process layer on top only OVER-subscribes and runs slower. (This
    # also means the predict is inherently non-deterministic at the raw-float level: two serial
    # runs differ by ~5e-17 due to parallel tree-sum order, identical to the emitted 3 decimals.
    # CRISTA output has therefore always been reproducible only to the emitted precision, and
    # CRISPRME_CRISTA_PARALLEL's feature-build parallelism preserves exactly that.)
    predictions = []
