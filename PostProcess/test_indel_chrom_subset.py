...
Commit 0294fc6b refactored the subset from
...
100% of indel off-targets were silently dropped (exit 0, clean logs) from
v2.1.9 onward.
...
class IndelChromSubset(unittest.TestCase):
    ...
    def test_buggy_grep_on_chrom_returns_nothing(self):
        """grep -F -w "$chrom" (the 0294fc6b bug) yields an EMPTY subset."""
        with tempfile.TemporaryDirectory() as td:
            targets = _targets_file(td)
            r = subprocess.run(
                ["bash", "-c",
