...
# "$fakechrom". Matching on "$chrom" (as a broken refactor did, see below) can
# never hit "fakechr22" with -w because the left word boundary fails ("chr22" is
# preceded by the word character "e"), so the subset comes back empty and every
# indel off-target is silently dropped with a clean exit (GitHub issue #172,
# regressed by commit 0294fc6b). -w still safely prevents "fakechr2" from
# matching "fakechr22" because the right boundary there is a digit.
